        ...
        foreach ($list as &$item) {
            ...
            foreach($salesman_user_result as $v):
                if ( $orderPaymentTime != '') {
                    ...
                    $salesman_amount += $StoreOrderServices->totalSalesByUser($v['uid'],strtotime($startTime),strtotime($endTime) + 24 * 3600);
                }else{
                    $salesman_orders += $StoreOrderServices->storeOrderCountByUser($v['uid'],'uid');
                    $salesman_amount += $StoreOrderServices->totalSalesByUser($v['uid']);
                }
            endforeach;
            ...
            //业务员绑定用户的下单总金额
            $item['salesman_amount'] = $salesman_amount;
            $item['salesman_award'] = 1200;
        }
